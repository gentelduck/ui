...
export async function build_registry_themes_item(colors_data: Record<string, any>): Promise<void> {
  ...
  for (const base_color of ['slate', 'gray', 'zinc', 'neutral', 'stone']) {
    ...
    await fs.writeFile(path.join(target_path, `${payload.name}.json`), JSON.stringify(payload, null, 2), 'utf8')
  }
}
