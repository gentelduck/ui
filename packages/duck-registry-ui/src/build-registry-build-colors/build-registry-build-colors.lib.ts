...
import { registry_base_colors, registry_color_mapping, registry_colors } from '@/registry'
...
export async function registry_build_colors_index(
  ...
  await fs.writeFile(path.join(colors_target_path, 'index.json'), JSON.stringify(colors_data, null, 2), 'utf8')
}

// ----------------------------------------------------------------------------
export async function build_registry_colors_base(colors_data: Record<string, any>): Promise<void> {
    ...
    for (const [mode, values] of Object.entries(registry_color_mapping)) {
      ...
      for (const [key, value] of Object.entries(values)) {
        if (typeof value === 'string') {
          ...
          const color = scale
            ? colors_data[resolved_base].find((item: any) => item.scale === parseInt(scale))
...
export async function build_registry_colors_themes() {
  ...
  await fs.writeFile(path.join(REGISTRY_PATH, `themes.css`), theme_css.join('\n'), 'utf8')
}
