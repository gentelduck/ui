    ...
    >
      ...
      <AccordionItem value="item-1">
        <AccordionTrigger
          ...
          icon={{ children: FaSortDown as LucideIcon, className: '!size-[20px]' }}
        >
        ...
        <AccordionContent>
          <ToggleGroup type="single">
            <ul className="flex items-center justify-center gap-4 p-4">
              {[
                { value: 'mastercard', label: 'Mastercard', icon: <TbBrandMastercard />, description: 'Credit card' },
                { value: 'visa', label: 'Visa', icon: <RiVisaLine />, description: 'Debit card' },
                { value: 'duckcard', label: 'Duckcard', icon: <BsCreditCard />, description: 'Credit card' },
      ...
      <AccordionItem value="item-2">
        ...
        <AccordionContent>
          <ToggleGroup type="single">
            <ul className="flex items-center justify-center gap-4 p-4">
              {[
                { value: 'paypal', label: 'PayPal', icon: <TbBrandPaypal />, description: 'Digital wallet' },
                { value: 'applepay', label: 'Apple Pay', icon: <FaApplePay />, description: 'Digital wallet' },
                { value: 'googlepay', label: 'Google Pay', icon: <FaGooglePay />, description: 'Digital wallet' },
              ].map(({ value, label, icon, description }) => (
                <ToggleGroupItem
                  key={value}
                  value={value}
