  ...
  React.useEffect(() => {
    triggerRef.current?.addEventListener('click', () => {
      ...
      if (!groupsRef.current.length || !itemsRef.current.length) {
        ...
        const items = wrapperRef.current?.querySelectorAll('[duck-dropdown-menu-item]') as never as HTMLLIElement[]
        groupsRef.current = Array.from(groups ?? []) as HTMLDivElement[]
        itemsRef.current = Array.from(items ?? []) as HTMLLIElement[]

        styleItem((itemsRef.current?.[0] as HTMLLIElement) ?? null)
        itemsRef.current?.[0]?.focus()
        selectedItemRef.current = (itemsRef.current?.[0] as HTMLLIElement) ?? null
      }
