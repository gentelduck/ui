...
const Image = React.forwardRef<HTMLImageElement, ImageProps>(({ wrapper, image, content }, ref) => {
  ...
  return (
    <Tooltip {...wrapper}>
      ...
      <TooltipContent
        className={cn('w-42 flex items-center justify-between gap-2', contentClassName)}
        {...contentProps}
      >
...
const ImageGroup = React.forwardRef<HTMLDivElement, ImageGroupProps>(({ wrapper, imgs, max_imgs }, ref) => {
        ...
        return (
          idx < max && (
            <Image
              image={{
                ...imgProps,
                className: cn('w-8 h-8 rounded-md border-muted-foreground/80 cursor-pointer', className),
              }}
