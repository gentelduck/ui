...
const InputCustomView = React.forwardRef<React.ElementRef<typeof Input>, InputCustomViewProps>(
  ({ trigger, label, badge }, ref) => {
    const { children: badgeChildren, className: badgeClassName, ...badgeProps } = badge ?? {}
    const { children: labelChildren, className: labelClassName, ...labelProps } = label ?? {}
