...
|----------------------------|----------|-------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `shouldScaleBackground`     | boolean  | `true`            | When `true`, scales the background when the drawer is open. Requires an element with the `[vaul-drawer-wrapper]` data attribute to handle background scaling.                                                                                                                               |
...
| `scrollLockTimeout`         | number   | `500ms`           | The time in milliseconds during which the drawer remains non-draggable after scrolling content within it. This prevents accidental dragging while scrolling inside the drawer.                                                                                                              |
...
This MDX table includes all the necessary props, types, default values, and descriptions for the `Drawer` component API reference.
